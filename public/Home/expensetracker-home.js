window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if(token != null){  
                ...
                // await axios.get('http://54.167.151.207:3000/get-all-expenses', {
...
async function getExpense(page) {
    ...
    const dbData = await axios.get(`http://54.167.151.207:3000/get-all-expenses?page=${page}&count=${count}`, {
        headers: {'authorization': token}
    })
...
async function saveExpenseToDatabase(e){
    ...
    try{
        ...
        await axios.post('http://54.167.151.207:3000/add-expense', obj, {
            headers: {'authorization': token}
        })
        .then(res => {
...
async function deleteExpense(id){
    ...
    try{
        ...
        await axios.delete(`http://54.167.151.207:3000/delete-expense/${id}`, {
            headers: {'authorization': token}
        })
        .then(res => {
...
document.getElementById('razorpay-button').onclick = async (e) => {
    try{
        const token = localStorage.getItem('token')
        const response = await axios.get('http://54.167.151.207:3000/purchase/premium', { headers: {'authorization': token}}) //Informing backend that a user wants to buy premium and in backend order_id is created and sent, which gets stored in const response variable here
        ...
        let options = {
            ...
            "handler": async (res) => {
                const backEndRes = await axios.post('http://54.167.151.207:3000/updateTransactionStatus', {
        ...
        rzp.on('payment.failed', async (res) => {
            ...
            await axios.post('http://54.167.151.207:3000/updateTransactionStatus/failed', {
                order_id: order_id
                }, { headers: {'authorization': token }})
...
document.getElementById('download-expenses-button').onclick = async (e) => {
    try{
        const token = localStorage.getItem('token')
        const response = await axios.get('http://54.167.151.207:3000/download-expense', {
            headers: {'authorization': token}
        })
...
document.getElementById('show-old-downloads-button').onclick = async (e) => {
    try{
            ...
            const oldDownloads = await axios.get('http://54.167.151.207:3000/get-old-downloads', {
                headers: {'authorization': token}
            })
